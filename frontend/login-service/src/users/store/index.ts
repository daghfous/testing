...
import { IRoleAction, ITimeoutConfiguration, IUnknownObjectKeys, IUser } from '../../interfaces/Interfaces'
 ...
 */
export const loginServiceUsersStore = defineStore('loginServiceUsers', () => {
  const { notifySuccess, notifyError } = notificationStore()

  // #####################################
  // STATE
  // #####################################
  const configuration = ref<ITimeoutConfiguration>({} as ITimeoutConfiguration)
  ...
  // GETTERS
  // #####################################
  const getConfiguration = computed((): ITimeoutConfiguration => configuration.value)
   ...
   * @param {object} user - user to update password.
   * @returns {Promise} - dispatch function.
   */
  ...
  const logout = async () => {
    try {
      const resp: IUnknownObjectKeys = await usersClient.delete('logout', {
        ...
        params: { relay_state: `${window.location.origin}/${window.location.pathname.split('/')[1]}` }
      })
      AuthStorageService.clearAuthAndUserDataFromStorages()
